...
const code = fs.readFileSync(path.resolve("contracts/TrustGraph.py"), "utf8");
...
if (!address || receipt.result_name !== "MAJORITY_AGREE" || fatal.length) {
  throw new Error(JSON.stringify({ address, consensus: receipt.result_name, fatal }));
}
console.log(JSON.stringify({ contractAddress: address, deploymentTransaction: hash,
  ...
  transactionExplorer: `https://explorer-studio.genlayer.com/tx/${hash}` }, null, 2));
